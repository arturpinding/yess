...
### Development phone-camera trust boundary

`npm run dev:phone` deliberately expands the development web server from loopback to `0.0.0.0` (all host interfaces) while advertising and allow-listing one validated private LAN origin. Use it only on a trusted network: the development app includes a no-login admin view, and a CSRF token is not operator authentication. The local FFmpeg/media-provider service remains on loopback port 8090 and must never be exposed for this workflow.

The launcher creates a constrained short-lived local CA and TLS certificate so Android can use camera/microphone APIs over HTTPS. Private keys remain under ignored `.local-certificates/` in 0700 directories with 0600 mode. The auxiliary HTTP server binds only to the selected host and returns only the public CA certificate or a minimal health response; private keys and directory contents have no route. Nevertheless, installing any user CA lets certificates signed by that CA become trusted on the device. Compare the displayed SHA-256 fingerprint on a trusted LAN, keep the development computer secure, and use a disposable computer browser/profile: the viewer must either trust the same CA or explicitly accept an exception for the exact development origin. Remove **RADA Phone Demo CA** from Android and any computer/browser trust store, remove saved browser exceptions after the demonstration, and delete local private material when it is no longer needed.

When phone broadcasting is available, `Permissions-Policy` permits `camera=(self)` and `microphone=(self)` only on the exact `/et/broadcast` and `/en/broadcast` pages. The viewer and every general route keep both features disabled; production requires an explicit feature opt-in. The broadcaster requests media only after an explicit button press. In `direct` mode its camera/microphone tracks travel directly to one viewer over WebRTC; the RADA server handles bounded setup messages, does not receive media and does not record it. Access credentials must not enter URLs or persistent browser storage.

Because `Permissions-Policy` belongs to the loaded document, navigation into or out of the exact capture page uses a full document request rather than a Next.js client transition. A phone session uses its server-returned expiry, currently 30 minutes; expiry stops camera/microphone tracks and the peer connection and attempts deletion. Stop, navigation and shutdown deletion are best-effort. An expired row is unusable immediately, but if deletion is not delivered its bounded offer/answer SDP and peer host candidates remain at rest until the exact expired room is accessed, any later development signaling request performs the opportunistic purge, or an operator removes it. There is no independent periodic signaling-row janitor.

This is not an Internet broadcasting boundary. Only direct host candidates are exchanged—no external STUN host is hard-coded and there is no TURN relay—so private addresses are disclosed to the participating peer and same-LAN connectivity is expected. Guest-network/client isolation, VPNs, firewalls and NAT can prevent connection. A production design needs authenticated broadcasters/viewers, authorization/rights enforcement, TURN/SFU capacity, abuse controls, recording policy, moderation/incident handling and separate production certificates; the direct routes remain disabled in production by default.

### Managed phone-camera trust boundary

With `PHONE_BROADCAST_PROVIDER=livekit-cloud`, the phone publishes camera and microphone tracks to a LiveKit Cloud room and viewers subscribe through LiveKit Cloud's SFU. Vercel is the control/token server: it authorizes creation, signs room access tokens, stores broadcast metadata and returns the configured project URL, but it does not proxy the media. LiveKit is a media processor and receives the live content plus connection metadata. This repository does not start recording or Egress. Before filming real people, establish the appropriate legal basis/notices and media rights, approve the processor agreement and data locations, and define incident/retention procedures.

`LIVEKIT_API_KEY` and `LIVEKIT_API_SECRET` are configured only in Vercel; the secret is never sent to either browser. The API key identifies the signer in the JWT issuer claim, so it is not a substitute for the secret. Vercel signs a room-bound publisher JWT that can publish but not subscribe or publish data; each viewer lookup receives a fresh room-bound JWT that can subscribe but not publish or publish data. Initial-connect TTL is at most 10 minutes and cannot exceed the broadcast's remaining life. The separate RADA `publisherToken` authorizes status/stop calls, is returned only to the phone and is stored only as a SHA-256 digest. Active broadcasts are listed without viewer authentication, so anyone who can reach the viewing page can obtain a viewer token for this prototype. The shared key, short code and process-local limiter are not substitutes for staff identity, RBAC, audience authorization, moderation or distributed abuse protection.

Stop disconnects the phone, revokes/removes its deterministic LiveKit participant and requests deletion of the room. A missing participant/room is idempotent success. The database records successful deletion; a transient failure leaves the terminal row cleanup-pending. Later managed-broadcast API traffic retries at most five pending rooms per request with a 30-second per-room backoff. Page shutdown is best effort, so six-hour expiry is also reconciled opportunistically on later traffic. There is no scheduled provider reconciliation job; provider inventory monitoring and an independent cleanup job are required before paid or unattended operation.

Treat the LiveKit API secret and broadcaster access key as credentials from the moment they are created. If either is pasted into a chat, issue, log or screen recording, revoke it in LiveKit/Vercel, create a replacement, update the secret store and redeploy. Do not rely on deleting the message as credential rotation. When `livekit-cloud` is selected, the CSP allows `https://*.livekit.cloud` and `wss://*.livekit.cloud` because the SDK discovers provider-selected regional hosts; that wildcard is absent from the `direct` configuration. The configured project URL itself is restricted to a root `wss://*.livekit.cloud` endpoint.
