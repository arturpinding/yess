...
In development, `Permissions-Policy` permits `camera=(self)` and `microphone=(self)` only on the exact `/et/broadcast` and `/en/broadcast` pages. The viewer and every general route keep both features disabled; production keeps the global denial. The broadcaster requests media only after an explicit button press. Its camera/microphone tracks travel directly to one viewer over WebRTC; the RADA server handles bounded setup messages, does not receive media and does not record it. Access credentials must not enter URLs or persistent browser storage.
